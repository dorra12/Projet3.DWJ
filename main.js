...
dddddd.addEventListener("click", function() {
    ...
    if ((ddd.currentFirstName.validity.valueMissing) || (ddd.currentName.validity.valueMissing)) {
        ddd.currentName.textContent = "Champs obligatoire";
        ddd.currentName.style.color = "red";
        ddd.currentName.style.fontSize = "10px";
    } else {
        ...
        sessionStorage.setItem("storageFirstName", ddd.currentFirstName.value);
        //stocker (sessionStorage) station Vélos actuelle
        currentStation = JSON.parse(sessionStorage.getItem('StationStorage'));
        debugger
        // instantier la class de Timer + stocker(sessionStorage)
        var counterTimer = new Timer(20);
        counterTimer.startCounter();

        ddd.currentReservation.textContent = "Vélo reservé à la station :" + " " + currentStation.address + " par " + currentStorageName + " " + currentStorageFirstName;
    }
})
...
if (sessionStorage.getItem("storageName")) {
    ...
    counterTimer.startCounter();
    /*counterTimer.startCounter();*/
    ddd.currentReservation.textContent = "Vélo reservé à la station :" + " " + JSON.parse(sessionStorage.getItem('StationStorage')).address + " par " + sessionStorage.getItem("storageName") + " " + sessionStorage.getItem("storageFirstName");
}
